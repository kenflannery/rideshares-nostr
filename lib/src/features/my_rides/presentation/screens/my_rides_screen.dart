  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('My Posted Rides', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, shadows: [Shadow(color: Colors.black26, offset: Offset(1, 1), blurRadius: 2)])),
        actions: [
      ...
      body: Consumer3<NostrService, MyRidesProvider, AuthService>(
        builder: (context, nostrService, ridesProvider, authService, child) {
          // Logged-in info section (always show if logged in)
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: authService.npub == null
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Npub copied to clipboard')));
                              },
                            ),
                          ],
                        ),
                          ...
                          builder: (context, authService, child) {
                            return Row(
                              children: [
                                ...
                                IconButton(
                                  icon: Icon(authService.showNsec ? Icons.visibility_off : Icons.visibility, size: 16),
                                  onPressed: () {
                                ...
                                IconButton(
                                  ...
                                  onPressed: authService.nsec == null || !authService.showNsec
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nsec copied to clipboard')));
                                  },
                                ),
                              ],
                            );
          ...
          if (nostrService.connectionState == NostrConnectionState.disconnected && !ridesProvider.isLoading) {
            ...
            rideContent = Center(
              child: FadeIn(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(color: Colors.blueAccent), SizedBox(height: 10), Text("Reconnecting to Nostr relays...", style: TextStyle(color: Colors.blueAccent))]),
              ),
            );
            ...
            rideContent = Center(
              child: FadeIn(
                duration: Duration(milliseconds: 300),
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(color: Colors.blueAccent), SizedBox(height: 10), Text("Loading your rides...", style: TextStyle(color: Colors.blueAccent))]),
              ),
            );
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text('Error: ${ridesProvider.error}', style: const TextStyle(fontSize: 16), textAlign: TextAlign.center),
                        const SizedBox(height: 20),
                        ElevatedButton(
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: Icon(ride.type == RideType.offer ? Icons.drive_eta : Icons.hail, color: Colors.white, size: 20),
        ),
        title: Text(
          ...
          children: [
            ...
            if (ride.description.isNotEmpty)
              Padding(
                ...
                child: Text(ride.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.grey[600])),
              ),
          ],
        ),
          ...
          onSelected: (String result) async {
            ...
            if (result == 'sold') {
              ...
              if (confirm) {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Ride marked as sold.' : 'Failed to mark ride as sold.')));
                }
              }
            } else if (result == 'edit') {
              ...
              if (confirm) {
                ...
                if (context.mounted && success != true) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to edit ride.')));
                }
              }
              ...
              if (confirm) {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Ride deleted.' : 'Failed to delete ride.')));
                }
              }
            }
          },
          itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
            if (ride.status == RideStatus.active)
              const PopupMenuItem<String>(
                value: 'sold',
                child: ListTile(leading: Icon(Icons.check_circle_outline, color: Colors.green), title: Text('Mark as Sold')),
              ),
            if (ride.status == RideStatus.active)
              const PopupMenuItem<String>(
                value: 'edit',
